...
function priceLabels(model: PricingModel, quotaPerUnit: number) {
    if (model.billing_mode === "tiered_expr" && model.billing_expr) {
        ...
        const labels = [...new Set(matches.map(([, token, value]) => `${TOKEN_LABELS[token]} ${money(Number(value))}/1M`))];
        if (labels.length) return labels;
    }

    const input = model.model_ratio * (1_000_000 / quotaPerUnit);
    return [`Input ${money(input)}/1M`, `Output ${money(input * model.completion_ratio)}/1M`];
}
...
export default function ModelPricingTab() {
    ...
    useEffect(() => {
        fetch("/api/model-pricing")
            .then(async (response) => {
                ...
                if (!response.ok || payload.success !== true) throw new Error(payload.message || "Pricing is unavailable.");
                setPricing(payload);
            })
            .catch((reason) => setError(reason instanceof Error ? reason.message : "Pricing is unavailable."));
    }, []);
    ...
    if (!pricing) return <p style={{ color: '#888' }}>Loading current model pricing…</p>;

    return (
        <section aria-labelledby="model-pricing-heading">
            <h2 id="model-pricing-heading" style={{ fontSize: '24px', color: '#fff', marginBottom: '12px' }}>Models & Pricing</h2>
            <p style={{ color: '#888', marginBottom: '24px' }}>
                Live gateway pricing for {pricing.data.length} models. Token prices are in USD per 1 million tokens and may vary by modality or cache tier.
            </p>
            <label style={{ display: 'block', marginBottom: '20px' }}>
                <span style={{ display: 'block', color: '#aaa', marginBottom: '8px', fontSize: '14px' }}>Search models</span>
                <input
                    type="search"
                    value={query}
            ...
            <p style={{ color: '#666', fontSize: '13px', marginBottom: '12px' }}>
                Showing {models.length} models · Pricing version {pricing.pricing_version}
            </p>
            <div style={{ overflowX: 'auto', marginBottom: '48px' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #333' }}>
                    <thead style={{ background: '#1a1a1a' }}>
                        <tr>
                            {['Model', 'Provider', 'Pricing', 'Endpoints'].map((heading) => (
                        ...
                        {models.map((model) => (
                            <tr key={model.model_name}>
                                ...
                                <td style={{ padding: '12px', border: '1px solid #333' }}>
                                    {priceLabels(model, pricing.quota_per_unit).map((label) => <div key={label}>{label}</div>)}
                                </td>
