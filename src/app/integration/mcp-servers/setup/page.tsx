...
\`\`\`
claude mcp add aporto --transport http https://api.aporto.tech/v1/mcp
\`\`\`

Add to your Claude Code settings file (\`~/.claude/settings.json\` or project \`.claude/settings.json\`):

\`\`\`
{  "mcpServers": {    "aporto": {      "transport": "http",      "url": "https://api.aporto.tech/v1/mcp"    }  }}
\`\`\`
...
If your platform requires explicit authentication, add \`--header "x-api-key: YOUR_API_KEY"\` (CLI) or a \`"headers": { "x-api-key": "YOUR_API_KEY" }\` block (JSON). Get a key at [app.aporto.tech/settings](https://app.aporto.tech/settings).
...
Verify your API key is correct and included in the \`x-api-key\` header. Get a new key at [app.aporto.tech/settings](https://app.aporto.tech/settings).

### Tools not appearing

[Section titled “Tools not appearing”](#tools-not-appearing)

Restart your AI client after adding the MCP server configuration. Verify the URL is exactly \`https://api.aporto.tech/v1/mcp\`.
